...
def pikachu():
    ...
    total += catalogoL["PikaRoll"]
    sushi += 1
    cantP += contadorL["sushiP"]
    return (sushi, cantP, total)

def otaku():
    global sushi, total, cantO
    total += catalogoL["OtakuRoll"]
    sushi += 1
    cantO += contadorL["sushiO"]
    return (sushi, cantO, total)

def pulpo():
    global sushi, total, cantV
    total += catalogoL["PulpoRoll"]
    sushi += 1
    cantV += contadorL["sushiV"]
    return (sushi, cantV, total)

def anguila():
    global sushi, total, cantA
    total += catalogoL["AnguilaRoll"]
    sushi += 1
    cantA += contadorL["sushiA"]
